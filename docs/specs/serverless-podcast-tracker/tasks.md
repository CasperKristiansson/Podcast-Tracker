...
- [ ] T-serverless-podcast-tracker-01: Monorepo scaffold (45m)
...
- [ ] T-serverless-podcast-tracker-02: TypeScript strict + ESLint baseline (45m)
...
- [ ] T-serverless-podcast-tracker-04: CDK bootstrap (both regions) runbook (30m)
  ...
  - Key edits: Commands with `--profile Personal`, region notes, prerequisites.
  - Dependencies: T-03.
  - Estimate: 30m
...
- [ ] T-serverless-podcast-tracker-07: Route 53 alias for podcast.casperkristiansson.com (30m)
...
- [ ] T-serverless-podcast-tracker-08: ConfigStack for SSM parameters (30m)
  - Goal: Define SSM paths for secrets and config (Spotify client ID/secret, environment flags).
  - Files/paths: `/infra/lib/ConfigStack.ts`.
  - Key edits: SSM standard-tier params with AWS-managed KMS; naming like `/podcast/{env}/spotify/{client_id|client_secret}`.
  - Dependencies: T-03.
  - Estimate: 30m
...
- [ ] T-serverless-podcast-tracker-21: Feature flags / guarded routes (30m)
  - Goal: Add simple env flags (e.g., `SPOTIFY_ENABLED`, `AUTH_READY`) to hide UI routes until backend ready.
...
- [ ] T-serverless-podcast-tracker-24: CI — deploy-infra.yml (60m)
  - Goal: `cdk synth` and `cdk deploy --require-approval never` to dev on push to `main`.
