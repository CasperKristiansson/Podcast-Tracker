import { clientId, cognitoDomain, identityProvider, redirectUri } from './config';
...
const buildTokenRequestBody = (code: string, codeVerifier: string): URLSearchParams => {
...
const buildTokenEndpointUrl = (): string => `https://${cognitoDomain}${TOKEN_ENDPOINT}`;
...
export const completeLogin = async (url: string = window.location.href): Promise<CallbackResult> => {
  if (typeof window === 'undefined') {
  ...
  if (error) {
    return {
      status: 'error',
      message: errorDescription ? `${error}: ${decodeURIComponent(errorDescription)}` : error
    };
  }
